...
/*/{Protheus.doc} PL210A
    MATA410 - EXECAUTO
    PL210A - GERAÇÃO DE PEDIDO DE VENDA
	Params: aItens = Itens a faturar	
	{CODPED[1], CLIENT[2], LOJA[3], PRODUT[4], DTENTR[5], HRENTR[6], QTDE[7], NATUR[8], GRUPV[9]}
/*/
User Function PL210A(aPedidos)
	Local aArea   		:= GetArea()
...
Static Function CriarPedidos(oSay)
	...
	// ORDER BY DTENTR, NATUR, HRENTR, GRUPV, PRODUTO
...
/*--------------------------------------------------------------------------
   Atualiza o status no arquivo para os pedidos criados
/*-------------------------------------------------------------------------*/
