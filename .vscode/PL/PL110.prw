...
Static Function etqproci(nNumEtq, nQtdeEmb)
	...
	For nX := 1 to nQtde
		...
		aAdd(aZPL, "^FT20,70^A0,40^FDMETALREZENDE^FS" )
		aAdd(aZPL, "^FT600,70^A0,40^FDPROCESSO^FS")

		// GB --> FOLeft,top ---   GBwidth,high,dots
		aAdd(aZPL, "^FO20,108^GB270,96,1^FS")
		aAdd(aZPL, "^FT30,130^A0,17^FDItem^FS")
		aAdd(aZPL, "^FT30,180^A0,50^FD" + cItem + "^FS")
		aAdd(aZPL, "^FO295,108^GB495,96,1^FS")
		aAdd(aZPL, "^FT340,180^A0,50^FD" + Substr(cDescr,1,20) + "^FS")
		...
		aAdd(aZPL, "^FT330,280^A0,35^FD" + cData + "^FS")
		aAdd(aZPL, "^FO540,210^GB250,96,1^FS")
		aAdd(aZPL, "^FT550,230^A0,17^FDOrdem^FS")
		aAdd(aZPL, "^FT570,280^A0,45^FD" + cNumOp + "^FS")
